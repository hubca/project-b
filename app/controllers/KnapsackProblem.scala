...
object KnapsackProblem {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val mediumLst = List(55, 85, 131, 440, 39, 138, 349, 165, 205, 95, 156, 157, 162, 159, 342, 58, 414, 370, 178, 505, 111, 327, 330, 163, 156, 168, 168, 194, 153, 54, 238, 110, 127, 254, 270, 293, 165, 58, 51, 107, 222, 239, 52, 54, 208, 162, 199, 159, 158, 165, 156, 168, 217)
    val mediumStr = Stream(156, 108, 125, 58, 211, 162, 163, 52, 208, 181, 205, 68, 297, 131, 162, 197, 111, 166, 55, 131, 440, 138, 349, 165, 205)
    ...
    try {
      ...
      executeTimePrint(method5(mediumLst, limit))
      //executeTimePrint(method6(mediumStr, limit).toList)
    } catch {
  ...
  def method4(arr: Array[Int], limit: Int): Array[Int] = {
    ...
    subset match {
      ...
      case _ => val res = (1 to subset.length).view
        .flatMap(subset.combinations)
        .find(_.sum == limit)
        if (res.isEmpty) method4(subset, limit-1)
        else res.get
    }
  }

  // tail recursive solution with pattern matching (update to method4) but using list
  def method5(lst: List[Int], limit: Int): List[Int] = {
    val subset = lst.filter(limit.>=)
    ...
    subset match {
      ...
      case _ => val res = (1 to subset.length).view
        .flatMap(subset.combinations)
        .find(_.sum == limit)
        if (res.isEmpty) method5(subset, limit-1)
        else res.get
    }
  }

  // tail recursive solution with pattern matching (update to method4) but using stream
  def method6(str: Stream[Int], limit: Int): Stream[Int] = {
    val subset = str.filter(limit.>=)
    ...
    subset match {
      ...
      case _ => val res = (1 to subset.length).view
        .flatMap(subset.combinations)
        .find(_.sum == limit)
        if (res.isEmpty) method6(subset, limit-1)
        else res.get
    }
  }
